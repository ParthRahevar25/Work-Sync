...
const AttendanceCard = () => {
  const { status, seconds, startTimer, pauseTimer, stopTimer } = useAttendance();
    ...
    <div className="bg-slate-900 rounded-3xl p-6 shadow-xl border border-slate-800 text-white h-full flex flex-col justify-between">
      <div className="flex justify-between items-center mb-10 border-b border-slate-800 pb-8">
        ...
        <div className="relative flex items-center justify-center">
            <div className={`absolute h-4 w-4 rounded-full animate-ping opacity-20 ${status === 'working' ? 'bg-emerald-500' : 'bg-amber-500'}`}></div>
            <div className={`h-2.5 w-2.5 rounded-full border-2 border-[#0F172A] z-10 ${status === 'working' ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.6)]' : 'bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.6)]'}`}></div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center">
        <div className="text-center py-10 bg-slate-900/50 rounded-[2rem] border border-slate-800 shadow-inner mb-10 group hover:border-blue-500/30 transition-all">
          ...
          <div className="flex items-center justify-center gap-2 mt-3">
            ...
            <p className="text-[11px] text-slate-500 uppercase font-black tracking-widest">Elapsed Work Hours</p>
          </div>
        </div>
        ...
        <div className="grid gap-4">
          {status === 'idle' ? (
            ...
            >
              {loading ? <Loader2 className="animate-spin" /> : <Play size={22} fill="currentColor" />} Check In
            </button>
          ) : (
            <div className="grid grid-cols-2 gap-4">
              <button 
                onClick={() => status === 'working' ? pauseTimer() : startTimer()} 
              ...
              >
                {status === 'working' ? <><Pause size={18} /> Break</> : <><Play size={18} /> Resume</>}
              </button>
              ...
              >
                {loading ? <Loader2 className="animate-spin" /> : <Square size={16} fill="currentColor" />} Finish
              </button>
            </div>
          )}
        </div>
