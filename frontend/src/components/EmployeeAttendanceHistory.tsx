...
const EmployeeAttendanceHistory = ({ refresh }: HistoryProps) => {
        ...
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-500 text-xs font-bold tracking-widest uppercase mt-1 opacity-80">
                Performance & Hour Distribution
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-6">
        {history.length > 0 ? (
          history.slice(0, 14).map((record) => {
            ...
            const dayNum = new Date(record.date).toLocaleDateString("en-US", {
              day: "numeric",
            });
            ...
            return (
              ...
              >
                ...
                <div
                  className={`absolute top-4 right-4 h-2 w-2 rounded-full shadow-[0_0_10px_rgba(0,0,0,0.5)] ${
                    isTargetMet
                ...
                <div className="flex flex-col items-center relative z-10">
                  ...
                  {/* Midnight Work Gauge */}
                  <div
                    className={`relative flex items-center justify-center h-16 w-16 rounded-full border-[3px] mb-6 transition-transform group-hover:scale-110 ${
                      isTargetMet
                  ...
                  >
                    <span
                      className={`text-base font-black ${isTargetMet ? "text-emerald-400" : "text-amber-400"}`}
                    >
                      {record.workHours?.toFixed(1) || 0}
                    </span>
                    <span className="absolute -bottom-2 text-[8px] font-black bg-[#0F172A] px-2 py-0.5 rounded-md uppercase text-slate-500 border border-slate-800">
                      Hrs
                    </span>
                  </div>
                  ...
                  <div className="w-full space-y-3">
                    ...
                    <div className="w-full bg-slate-800 h-1.5 rounded-full overflow-hidden p-0.5 border border-slate-700/50">
                      <div
                        ...
                        style={{
                          width: `${Math.min((record.workHours / 8) * 100, 100)}%`,
                        }}
                      ></div>
                    </div>
                  </div>
